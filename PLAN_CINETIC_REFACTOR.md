...
| **P6** ✅ | Docs: README, CLAUDE.md, PLAN files + remaining code branding + **full Italian-comment sweep**. | DONE — README/CLAUDE/PLAN rebranded (CINETIC backronym, unified-CLI usage, new modules); topology/engine/job-name `CRAB`/`crab_` branding fixed; every Italian comment across `src/`, entry scripts, `.tcss`, and `blink_plotter.py` translated to English; headless TUI + all 5 test suites + analyzer green. `tournament_nb.c` env name already done in P2 (needs a `make` rebuild to take effect — no MPI toolchain loaded here). |
| **P7** *(opt)* | A5 tree reshape + A6 schema versioning. | DEFERRED (2026-06-25) — left as a future task; the engine split is a high-risk verbatim move with no integration-test coverage, judged not worth it now. CINETIC is complete and shippable at P6. |
