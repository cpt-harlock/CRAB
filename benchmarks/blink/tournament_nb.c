...
static int exchange_window(int role, int partner, unsigned char *send_buf,
  ...
  if (role == 0) {
    MPI_Recv(ack, 4, MPI_BYTE, partner, TAG_ACK, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  } else {
...
int main(int argc, char **argv) {
  ...
  if (my_rank == master_rank) {
    ...
    if (endless) {
      printf(
          "Tournament with %d processes, msg-size: %d, window: %d, rounds: %d, "
  ...
  do {
    ...
    for (round = 0; round < num_rounds; round++) {
      ...
      for (s = 0; s < skip_iters; s++) {
        exchange_window(role, partner_rank, send_buf, recv_buf, window,
                        msg_size, timeout, reqs);
      }
      ...
      for (k = 0; k < max_iters; k++) {
        ...
        do {
          ...
          for (g = 0; g < measure_granularity; g++) {
            if (exchange_window(role, partner_rank, send_buf, recv_buf, window,
                                msg_size, timeout, reqs)) {
              timeouts++;
            }
          }
          durations[curr_iters % max_samples] =
              MPI_Wtime() -
          ...
          if (burst_length != 0) { /*bcast needed for synch if bursts timed*/
            if (my_rank ==
                master_rank) { /*master decides if burst should be continued*/
              burst_cont = ((MPI_Wtime() - burst_start_time) < burst_length);
            }
            MPI_Bcast(&burst_cont, 1, MPI_INT, master_rank,
                      MPI_COMM_WORLD); /*bcast the masters decision*/
          }
        } while (burst_cont);
        if (burst_pause != 0) {
  ...
  MPI_Reduce(&timeouts, &total_timeouts, 1, MPI_INT, MPI_SUM, master_rank,
             MPI_COMM_WORLD);
